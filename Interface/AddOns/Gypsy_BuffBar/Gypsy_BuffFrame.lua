...
Gypsy_BuffFrameCapsuleInterval = 13;
...
function Gypsy_BuffButtonUpdate()
	...
	if (buffName) then
		getglobal(this:GetName().."DescribeText");
	end
...
function Gypsy_BuffButtonUpdateAny(button)
	...
	if (buffName) then
		getglobal(button:GetName().."DescribeText");
	end
...
function Gypsy_GetStringTime(time)
	local string = "";
	if( time <= 0 ) then
		string = "";
