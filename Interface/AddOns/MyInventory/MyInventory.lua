...
MyInventoryColumns              = 6;

MyInventoryReplaceBags          = 0;
MyInventoryBags                 = "11111";
MyInventoryGraphics             = 0;
...
function MyInventoryFrame_OnLoad()
	...
	local pScale = MyInventoryFrame:GetParent():GetScale();
	if ( not pScale ) then pScale = 1; end
	if MyInventoryFrame:GetScale() ~= pScale * tonumber(MyInventoryScale) then
	...
	for j=1, frameSize, 1 do
		...
		if ( j == 1 ) then
			...
			if MyInventoryAIOIStyle == 1 then
				dx = (columns-1)*MYINVENTORY_ITEM_OFFSET_X + MYINVENTORY_FIRST_ITEM_OFFSET_X;
				dy =    (rows-1)*MYINVENTORY_ITEM_OFFSET_Y + First_Y;
			else
			...
			if ( mod((j-1), columns) == 0 ) then
				baseframe = name.."Item"..(j - columns);
				dx = 0;
				if MyInventoryAIOIStyle == 1 then
					dy = -MYINVENTORY_ITEM_OFFSET_Y;
				else
					dy = MYINVENTORY_ITEM_OFFSET_Y;
				end
			else
		...
		if MyInventory_ShouldOverrideBag(bag) then 
			for slot=1, GetContainerNumSlots(bag) do
				...
				if ( string.find(GetBagName(bag), "Quiver") or string.find(GetBagName(bag), "Ammo") ) then
					SetItemButtonNormalTextureVertexColor(itemButton, 0.0, 0.0, 0.0);
				end
				if ( texture ) then
