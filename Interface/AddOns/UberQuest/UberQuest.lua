...
UBERQUEST_VERSION = "2005.03.27" -- Notice the cleverly disguised date.

UIPanelWindows["UberQuest_List"] = { area = "left", pushable = 997 };
BINDING_HEADER_UBERQUEST_SEP = "Uber Quest";
BINDING_NAME_UBERQUEST_CONFIG = "Quest Log";
...
local UberQuest_PlayerClass = nil;
...
local function UberQuest_Minion_Reset()
...
local function UberQuest_ConfigInit()
	...
	if (UberQuest_Config.speedquest) then
...
function UberQuest_OnLoad()
	...
	-- No need to keep an old pointer, because we're never 
	-- gonna call it or release this override. TAKE THAT 
	-- EVIL BLIZZARD FUNCTION! I HAVE SLAIN THEE!
...
function UberQuest_OnEvent()
	if (event == "UNIT_NAME_UPDATE" and arg1 == "player") then
		...
		if (playerName ~= UKNOWNBEING and playerName ~= "Unknown Entity") then
			UberQuest_PlayerClass = UnitClass("player");
			UberQuest_Player = playerName;
			if (UberQuest_Player and UberQuest_Config_Loaded) then
...
function UberQuest_List_Update()
	...
	-- ScrollFrame update
	FauxScrollFrame_Update(UberQuest_List_Scroll, numEntries, UBERQUEST_QUESTS_DISPLAYED, UBERQUEST_QUESTLINE_HEIGHT, UberQuest_List_HighlightFrame, 293, 316 )
	...
	for i=1, UBERQUEST_QUESTS_DISPLAYED, 1 do
		...
		local questHighlightText = getglobal("UberQuest_List_Title"..i.."HighlightText");
		local questDisabledText = getglobal("UberQuest_List_Title"..i.."DisabledText");
...
function UberQuest_Config_Update()
	...
	UberQuest_ConfigFrame_SpeedQuest:SetChecked(UberQuest_Config.speedquest);
	if (UberQuest_Config.useminion) then
...
function UberQuest_MinionShowHide()
	if (UberQuest_Config and UberQuest_Player and UberQuest_Config[UberQuest_Player]) then
		if (UberQuest_Minion:IsVisible()) then
			UberQuest_Minion:Hide();
			UberQuest_List_SummonMinion:SetText("Summon Minion");
			UberQuest_Config[UberQuest_Player].minionvisible = nil;
		else
			UberQuest_Minion:Show();
			UberQuest_List_SummonMinion:SetText("Dismiss Minion");
			UberQuest_Minion_Update();
...
function UberQuest_Minion_Update()
	...
	if (UberQuest_Config and UberQuest_Player and UberQuest_Config[UberQuest_Player] and UberQuest_Config[UberQuest_Player].selected and UberQuest_Config[UberQuest_Player].selected ~= {}) then
		...
		while (questLogTitleText) do
			...
			elseif (UberQuest_Config[UberQuest_Player].selected[questLogTitleText]) then
				...
				if (isComplete) then
					...
					applytext = applytext.." |cff"..color.."(Complete)|r";
				else
		...
		if (applytext == "") then
			UberQuest_Minion_Text:SetText("No selected quests.");
		else
